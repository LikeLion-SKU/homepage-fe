...
export default function MemberCard({ data }) {
  return (
    <div className="flex flex-col w-40 h-fit pad:w-54 pad:h-fit web:w-59 border-2">
      ...
      />
      <div className="flex flex-col flex-1 px-3 pt-2 pad:px-6 pad:py-4.5 gap-1 pad:gap-2 bg-[#F8FBE7] border-t-2">
        <p className="text-[1.1rem] pad:text-[1.325rem] font-bold">{data.name}</p>
        <div className="flex flex-col">
          <p className="text-[0.7rem] pad:text-[1rem] truncate">{`${data.department}`}</p>
          <p className="text-[0.7rem] pad:text-[1rem]">{`${data.shortStudentNumber}학번`}</p>
        </div>
      </div>
    </div>
  );
